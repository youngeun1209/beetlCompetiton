...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 28 09:37:51 2021
...
testbase = savebase + 'finalSleep/testing/'

X_test = []
for subj in range(5, 14):
    for session in [1,2]:
        with open(testbase + "testing_s{}r{}X.npy".format(subj, session), 'rb') as f:
...
targetbase = savebase + 'finalSleep/sleep_target/'
...
for i_sub in [0,2]:
    for i_record in [1]:
        with open(osp.join(targetbase, "testing_s%dr%dX.npy" %(i_sub,i_record)), 'rb') as f:
            X_target.append(pickle.load(f))
        with open(osp.join(targetbase, "testing_s%dr%dy.npy" %(i_sub,i_record)), 'rb') as f:
...
for i_sub in [1,3,4]:
    for i_record in [1,2]:
        ...
        with open(osp.join(targetbase, "testing_s%dr%dy.npy" %(i_sub,i_record)), 'rb') as f:
            y_target.append(pickle.load(f))
X_train = np.concatenate(X_target)
y_train = np.concatenate(y_target)
print("Target: there are {} trials with {} electrodes and {} time samples".format(*X_train.shape))
...
optimizer = optim.SGD(model.parameters(), lr=1e-7, momentum=0.9, weight_decay=0.9)
# optimizer = optim.AdamW(model.parameters(), lr=1e-2, weight_decay=0.9)

lr_scheduler = StepLR(optimizer, step_size=100, gamma=0.1)

# %% transfer
savebase_log='/exSSD/projects/beetlCompetition/learning_logs/task1_Sleep/check_final2/'

checkpoint = torch.load(savebase_log+'DSN_model_sleep.pth')
...
# %%
savename = savebase_log + 'DSN_model_sleep_tuning.pth'
...
checkpoint = torch.load(savebase_log+'DSN_model_sleep_tuning.pth')
model.load_state_dict(checkpoint['model_state_dict'])
model.eval()

for setname, dataset in (('Valid', valid_set), ('Test', test_set)):
    ...
    if setname == 'Valid':
        ...
        predicted_labels = np.argmax((outputs), axis=1)
        accuracy = np.mean(dataset.y  == predicted_labels)
        print("{:6s} Accuracy: {:.5f}".format(setname, float(accuracy)))
        
        plot_confusion_matrix(dataset.y, predicted_labels, classes=['0', '1', '2', '3', '4', '5'], 
                              normalize=True, title='Validation confusion matrix')
                
        plt.savefig(savebase_log+'Tuning_valid_accuracy{:.3f}.png'.format(accuracy), dpi=300)
        plt.show()
    else:
...
np.savetxt(savebase_log+'pred_Sleep_label.txt',sleep_label,delimiter=',',fmt="%d")
